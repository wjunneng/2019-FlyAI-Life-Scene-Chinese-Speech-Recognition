...
from DFCNN_Transformer.util.util import TransformerUtil
...
class TransformerModel(object):
    def __init__(self, arg, acoustic_vocab_size, language_vocab_size):
        ...
        with self.graph.as_default():
            ...
            self.emb = TransformerUtil.embedding(self.x, vocab_size=self.input_vocab_size, num_units=self.hidden_units,
                                                 scale=True, scope="enc_embed")
            position_emb = TransformerUtil.embedding(
            ...
            for i in range(self.num_blocks):
                with tf.variable_scope("num_blocks_{}".format(i)):
                    # Multihead Attention
                    self.enc = TransformerUtil.multihead_attention(queries=self.enc,
                                                                   keys=self.enc,
                    ...
                    # Feed Forward
                    self.outputs = TransformerUtil.feedforward(self.enc,
                                                               num_units=[4 * self.hidden_units, self.hidden_units])

            # Final linear projection
            self.logits = tf.layers.dense(self.outputs, self.label_vocab_size)
            self.preds = tf.to_int32(tf.argmax(self.logits, axis=-1))
            self.istarget = tf.to_float(tf.not_equal(self.y, arg.PAD))
            ...
            if self.is_training:
                # Loss label平滑
                self.y_smoothed = TransformerUtil.label_smoothing(tf.one_hot(self.y, depth=self.label_vocab_size))
                # loss计算，预测结果与平滑值的交叉熵
